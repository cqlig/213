...
const TicketList = () => {
  ...
  const [statusFilter, setStatusFilter] = useState('all'); // 'all', 'valid', 'redeemed'
...
The change is to replace API calls to use relative paths.
```javascript
import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import axios from 'axios';
...
const TicketList = () => {
  ...
  return (
    <div>
      <div className="form-container">
        <h2>
          📋 Lista de Entradas ({filteredTickets.length} de {tickets.length})
        </h2>
        ...
        <div style={{ marginBottom: '30px' }}>
          ...
          <div className="form-group">
            ...
            >
              ...
              >
                ✅ Válidos ({
                  tickets.filter((t) => t.status === 'Válido').length
                })
              </button>
              <button
                onClick={() => setStatusFilter('redeemed')}
                className={`nav-button ${
                  statusFilter === 'redeemed' ? 'active' : ''
                }`}
                style={{
                  background:
              ...
              >
                🎫 Canjeados ({
                  tickets.filter((t) => t.status === 'Canjeado').length
                })
              </button>
            </div>
          </div>
        ...
        {tickets.length === 0 ? (
          <p>
            No hay entradas creadas aún. <Link to="/">Crea la primera entrada</Link>
          </p>
        ) : filteredTickets.length === 0 ? (
          <p>No se encontraron entradas que coincidan con tu búsqueda.</p>
        ) : (
          <div className="tickets-list">
            {filteredTickets.map((ticket) => (
              <div key={ticket.id} className="ticket-card">
                <h3>{ticket.buyer_name}</h3>
                <p>
                  <strong>Evento:</strong> {ticket.event_name}
                </p>
                {ticket.buyer_email && (
                  <p>
                    <strong>Correo:</strong> {ticket.buyer_email}
                  </p>
                )}
                <p>
          ...
          >
            ...
            <div id={`ticket-${selectedTicket.id}`} style={{ marginBottom: '20px' }}>
              <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                <h4>🎫 {selectedTicket.event_name}</h4>
                <p>
                  <strong>Comprador:</strong> {selectedTicket.buyer_name}
                </p>
                {selectedTicket.buyer_email && (
                  <p>
                    <strong>Correo:</strong> {selectedTicket.buyer_email}
                  </p>
                )}
                <p>
